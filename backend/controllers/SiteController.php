...
//                if (!file_exists($webroot.$dir_thumb)){
//                    self::makeDir($webroot.$dir_thumb);
//                }
                $timename=time().mt_rand(100,999) ;
                $name =$dir.'/' . $timename . '.' . $file->extension;
//                $name_thumb=$dir_thumb.'/' . $timename . '.' . $file->extension;
                $file->saveAs($webroot.$name);
                $b=1;
                $name1=str_replace("/upload/","",$name,$b);
//                \common\components\functional\ImageUtility::thumbs($webroot.$name,$webroot.$name_thumb,$width,$height);
                $url =Yii::$app->params['file_url'].$name1;
//                $url_thumb =Yii::$app->params['file_url'].$name_thumb;
                $result = [
                    'jsonrpc'=>'2.0',
                    'status'=>1,
                    'result'=>$url,
