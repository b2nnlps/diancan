...
//    $default_file_url = "http://" . $_SERVER['HTTP_HOST'] . "/upload/";
  //  $default_file_url = "http://localhost/msjlb_v2.0/frontend/web/fb_v1.0/upload/";
    $default_file_url = "http://ms.n39.cn/fb_v1.0/upload/";
//    $default_file_url="upload/";
    ...
    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_img, $result)) {
        ...
        $imgType="pjpeg,jpeg,jpg,gif,bmp,png";
        $imgFileType=explode(',',$imgType);
        $mess=array_unique($imgFileType);
        //in_array(value,array,type)
        $isin = in_array($type,$mess);//php判断数组元素中是否存在某个字符串的方法
        if($isin){
