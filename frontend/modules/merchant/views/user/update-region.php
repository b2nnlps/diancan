...
<div class="box"><!--头部：开始--->
    ...
    <div class="main">
        ...
        <div class="demox">
            <div class="opt">
                <input class="magic-checkbox" type="checkbox" name="layout" id="defaults" <?php if($model['default']==10){echo " checked=\"checked\""; }?>>
                <label for="defaults">默认地址</label>
            </div>
        </div>
        <div class="clear"></div>
        <div class="cellphone"><span>地址详情</span><input id="address" type="text" value="<?=$model['address']?>" placeholder="地址详情" /></div>
        <div class="cellphone">
            <span>手机号码</span><input id="phone" type="text" value="<?=$model['phone']?>" placeholder="联系人手机号码" />
        </div>

        <div class="cellphone"><span>邮政编码</span><input id="zipcode" type="text" value="<?=$model['zipcode']?>" placeholder="邮政编码" /></div>
...
//                }
                ...
                var data =csrfName + '='+crsfToken+"&consignee="+consignee.val()+"&phone="+phone.val()+"&defaults="+shows+"&address="+address.val()+"&zipcode="+zipcode.val()+"&originator="+originator+"&id="+<?=$model['id']?>;
//                alert(data);
                $.post("<?=\yii\helpers\Url::to(['save-region'])?>", data, function (result) {
                    if(result!==''){
                        $.dialog( 'alert','温馨提示', result, 4000,true);
                    }else{
                       window.location.href="<?=Yii::$app->urlManager->createAbsoluteUrl(['merchant/user/address'])?>";
