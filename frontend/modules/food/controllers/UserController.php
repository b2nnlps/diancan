...
class UserController extends BaseController
{
    ...
    public function actionIndex($shopId = 0, $table = 0, $food_id = 0)
    {
        ...
        if ($shopId) setcookie("shopId", $shopId, time() + 86400, "/"); else $shopId = $_COOKIE['shopId'];
        if ($table) setcookie("table", $table, time() + 86400, "/");
...
<span class="price_box">￥' . $info['price'] * $cart[$i]['num'] . '</span></dd></dl>';
    ...
    {//扫码，商家和桌号
        ...
        if ($userInfo['subscribe'] == 1) {//如果已关注直接跳转
            ...
            echo "<img src='$QRUrl'>$scene_id <br>您好，为了方便操作，请长按二维码关注公众号后点餐";
        }
