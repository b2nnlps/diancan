...
//    public $layout='main';
    ...
    public function beforeAction($action)
    {
        ...
        if($this->isWxBrowser &&!isset($_COOKIE['msjlb_openid']) && Yii::$app->controller->action->id != 'auth-redirect'){
            ...
            $redirect = Yii::$app->urlManager->createAbsoluteUrl(['activitys/relay/auth-redirect']).'?URL='.($_SERVER['REQUEST_URI']);
...
//                $wxuser->id=uniqid().rand(1000,9999);
                ...
                $wxuser->status=0;
                $wxuser->module=3;//所属模块微信用户，1：通用,2:e-shop,3：助力模块
...
//           Yii::$app->user->login($wxuser, 3600 * 24);
          //  setcookie('openId',$wxuser->openid,time()+3600*6,'/');
           setcookie('msjlb_openid',$wxuser->openid,time()+3600*24,'/');
        }
