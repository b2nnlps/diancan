...
//base64位图片上传
    public static function base64_upload($base64_img, $config = array()){
        
//        $default_up_dir = dirname(dirname(__FILE__));
        $default_up_dir = Yii::getAlias('@webroot');//E:/wamp/www/msjlb_v2.0/frontend/web
        $default_up_dir = str_replace("\\", "/", $default_up_dir)."/fb_v1.0/upload/";
        if (!file_exists($default_up_dir)) {
            mkdir($default_up_dir, 0775);
        }

        //    $default_file_url = "http://" . $_SERVER['HTTP_HOST'] . "/upload/";
//    $default_file_url = "http://localhost/msjlb_v2.0/frontend/web/fb_v1.0/upload/";
        $default_file_url = Url::base(true)."/fb_v1.0/upload/";//http://localhost/msjlb_v2.0/frontend/web/fb_v1.0/upload/
